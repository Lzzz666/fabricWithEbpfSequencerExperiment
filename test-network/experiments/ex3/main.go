...
func main() {
	...
	go func() {
		defer wg.Done()
		measureLatency(p0.Contract, numTx, perPeerRPS, fmt.Sprintf("latency_peer1_%d.csv", rps))
	}()

	go func() {
		defer wg.Done()
		measureLatency(p1.Contract, numTx, perPeerRPS, fmt.Sprintf("latency_peer2_%d.csv", rps))
	}()
